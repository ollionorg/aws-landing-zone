...
def lambda_handler(event, context):
    ...
    for log_group in log_groups:
        ...
        #if 'ExportToS3' not in log_group_tags and log_group_tags['ExportToS3'] == 'false':
    ...
    for log_group_name in log_groups_to_export:
        ...
        if export_to_time - int(ssm_value) < (0.5 * 60 * 60 * 1000):
            # Haven't been 24hrs from the last export of this log group
            print("    Skipped until 30 minutes from last export is completed")
            continue
        ...
        while max_retries > 0:
            try:
                response = logs.create_export_task(
                    ...
                    destinationPrefix=os.environ['AWS_ACCOUNT'] + '/' + os.environ['REGION'] + '/' + log_group_name.strip('/')
                )
