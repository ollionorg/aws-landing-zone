...
variable "security_tool_account_id" {
  description = "Management Account ID"
  type        = string
}
...
variable "sns_recipients_email_addresses" {
  description = "List of email addresses to verify so that they may receive emails when SNS is in sandbox mode. ONLY USE THIS FOR DEVELOPMENT PURPOSES!"
  type        = list(string)
  default     = []
}