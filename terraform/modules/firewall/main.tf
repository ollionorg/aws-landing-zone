...
resource "aws_networkfirewall_firewall_policy" "this" {
  name = "nfw-policy-${var.firewall_name}"
...
resource "aws_cloudwatch_log_group" "nfw" {
  for_each = var.log_destination_type == "CloudWatchLogs" ? try(var.logging_config, {}) : {}
  name     = "${var.logging_group_prefix}/${each.key}"
    ...
    dynamic "log_destination_config" {
      ...
      content {
        log_destination = var.log_destination_type == "CloudWatchLogs" ? {
          ...
          bucketName = var.log_destination_s3_bucket
          prefix     = "${var.log_destination_s3_prefix}/${log_destination_config.key}"
          } : {
          deliveryStream = var.kinesis_firehose_delivery_stream
        }
