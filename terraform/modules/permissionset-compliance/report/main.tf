locals {
  lambda_function_name       = "prmset_report_lambda_function"
...
data "archive_file" "log_exporter" {
  type        = "zip"
  source_file = "${path.module}/lambda/permissionset-report.py"
  output_path = "${path.module}/lambda/tmp/permissionset-report.zip"
}
...
data "aws_s3_bucket" "auditlog_bkt" {
  ...
  bucket   = var.permissionset_report_bucket
}



## IAM Policy and Role For Lambda
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:aws:s3:::${var.permissionset_report_bucket}",
        "arn:aws:s3:::${var.permissionset_report_bucket}/*"
      ],
      "Effect": "Allow"
    },
    ...
    },
    {
      ...
      "Resource": "arn:aws:iam::${data.aws_caller_identity.shared_service.account_id}:role/prmset_report_ses_cross_account_IAM_Role",
      "Effect": "Allow"
    }
  ]
}
...
resource "aws_lambda_function" "log_exporter" {
  ...
  handler          = "permissionset-report.lambda_handler"
  ...
  environment {
    variables = {
      S3_BUCKET           = var.permissionset_report_bucket,
      AWS_ACCOUNT         = data.aws_caller_identity.current.account_id
      REGION              = data.aws_region.current.name
      SES_SENDER_EMAIL    = var.ses_sender_email
      SES_RECIPIENT_EMAIL = local.ses_recipient_email_string
      SES_ROLE_ARN        = "arn:aws:iam::${data.aws_caller_identity.shared_service.account_id}:role/prmset_report_ses_cross_account_IAM_Role"
    }
  }
}

resource "aws_cloudwatch_event_rule" "log_exporter" {
  name                = "${local.lambda_function_name}_event_rule_${data.aws_region.current.name}"
...
resource "aws_lambda_invocation" "generate_report" {
  function_name = aws_lambda_function.log_exporter.function_name
...
resource "aws_iam_role" "ses_cross_account_role" {
  provider           = aws.shared_service
  name               = "prmset_report_ses_cross_account_IAM_Role"
