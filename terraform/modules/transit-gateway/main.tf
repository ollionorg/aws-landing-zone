locals {
  ...
  #   vpc_route_table_destination_cidr_igw = flatten([
  #     for k, v in var.vpc_attachments : [
  #       for rtb_id in try(v.vpc_route_table_ids_for_igw, []) : {
  #         rtb_id = rtb_id
  #         cidr   = v.igw_destination_cidr
  #       }
  #     ]
  #   ])

  #   vpc_route_table_destination_cidr_nat = flatten([
  #     for k, v in var.vpc_attachments : [
  #       for rtb_id in try(v.vpc_route_table_ids_for_nat, []) : {
  #         rtb_id = rtb_id
  #         cidr   = v.nat_destination_cidr
  #       }
  #     ]
  #   ])
}
...
resource "aws_ec2_transit_gateway_route_table" "this" {
  ...
  tags = merge(
    var.tags,
    { Name = var.tgw_additional_rt_name },
...
resource "aws_ec2_transit_gateway_route" "this" {
  ...
  transit_gateway_route_table_id = var.create_tgw ? try(local.vpc_attachments_with_routes[count.index][1].pre_inspection_route, false) ? aws_ec2_transit_gateway.this[0].association_default_route_table_id : aws_ec2_transit_gateway_route_table.this[0].id : var.transit_gateway_route_table_id
...
#   for_each = { for x in local.vpc_route_table_destination_cidr_igw : x.rtb_id => x.cidr }
...
#   for_each = { for x in local.vpc_route_table_destination_cidr_nat : x.rtb_id => x.cidr }
