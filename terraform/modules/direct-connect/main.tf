...
resource "aws_ec2_transit_gateway_route" "this" {
  count = length(var.direct_connect_static_routes_destinations)

  destination_cidr_block         = var.direct_connect_static_routes_destinations[count.index]
  transit_gateway_route_table_id = var.transit_gateway_route_table_id
  transit_gateway_attachment_id  = data.aws_ec2_transit_gateway_dx_gateway_attachment.this.*.id[0]
}

data "aws_ec2_transit_gateway_dx_gateway_attachment" "this" {
  depends_on         = [aws_dx_gateway_association.cross_account]
  count              = var.crossaccount_dx_gateway ? 1 : 0
  transit_gateway_id = var.vgw_id
  dx_gateway_id      = var.dx_gateway_id
}
