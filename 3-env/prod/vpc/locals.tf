locals {
  ...
  account_id                  = data.terraform_remote_state.remote.outputs.accounts_id_map.prod_master
  bu_app_prod_account_id      = data.terraform_remote_state.remote.outputs.accounts_id_map.bu1_app_prod
  ...
  subnets_cidr                = cidrsubnets(local.lz_config.env.prod.vpc_cidr, 4, 4, 4, 4, 4, 4)
