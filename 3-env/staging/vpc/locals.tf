locals {
  ...
  bu_app_stg_account_id       = data.terraform_remote_state.remote.outputs.accounts_id_map.bu1_app_stg
  ...
  subnets_cidr                = cidrsubnets(local.lz_config.env.staging.vpc_cidr, 4, 4, 4, 4, 4, 4)
