...
module "tgw-attachment" {
  ...
  vpc_attachments = {
    DNS = {
      vpc_id                 = data.terraform_remote_state.dns-hub-vpc.outputs.vpc_id
      subnet_ids             = data.terraform_remote_state.dns-hub-vpc.outputs.tgw_attachment_subnets
      dns_support            = true
      ipv6_support           = false
      ...
      vpc_route_table_ids  = [data.terraform_remote_state.dns-hub-vpc.outputs.tgw_attachment_route_table_ids[0], data.terraform_remote_state.dns-hub-vpc.outputs.private_route_table_ids[0]]
      tgw_destination_cidr = "0.0.0.0/0"

      tgw_routes = [
        {
          destination_cidr_block = data.terraform_remote_state.dns-hub-vpc.outputs.vpc_cidr_block
        },
        # {
        #   blackhole              = true
